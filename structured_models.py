...
L_neural = α_loc · S_θ^loc + α_coarse · P_θ C_θ R_θ

Memory: O(E + N·m) instead of O(N²).
All graph utilities are O(E) or O(N log N) — never O(N²).
"""
...
class MPNNLayer(MessagePassing):
    ...
    def message(self, x_i, x_j, edge_attr):
        return self.msg_mlp(torch.cat([x_i, x_j, edge_attr], -1))

    def update(self, aggr_out, x):
        return self.upd_mlp(torch.cat([x, aggr_out], -1))
...
# ══════════════════════════════════════════════════════════════
# Fine Graph Encoder (geometry only — NOT u^n)
# ══════════════════════════════════════════════════════════════
...
# ══════════════════════════════════════════════════════════════
# Local Correction S_θ
# ══════════════════════════════════════════════════════════════

class LocalCorrectionHead(nn.Module):
    """
    (S u)_i = Σ_j w_ij (u_j - u_i),  S·1 = 0.  NEVER dense N×N.
    """
...
def low_rank_matvec(P, R, c_w, coarse_ei, x, m):
    """(P C R) x."""
    Rx = R @ (x.unsqueeze(-1) if x.dim() == 1 else x)
    if Rx.dim() == 2 and Rx.shape[1] == 1:
        Rx = Rx.squeeze(-1)
    CRx = sparse_laplacian_matvec(c_w, coarse_ei, Rx, m)
    return (P @ (CRx.unsqueeze(-1) if CRx.dim() == 1 else CRx)).squeeze(-1) \
        if CRx.dim() == 1 else P @ CRx


def structured_L_matvec(x, Lp_w, fine_ei, s_w, a_loc,
                         P, R, c_w, coarse_ei, a_coarse, N, m):
    """L_θ x = L_phys x + α_loc S x + α_coarse P C R x."""
    out = sparse_laplacian_matvec(Lp_w, fine_ei, x, N)
    out = out + a_loc * sparse_laplacian_matvec(s_w, fine_ei, x, N)
    # low-rank part
    Rx = R @ x  # [m]
    CRx = sparse_laplacian_matvec(c_w, coarse_ei, Rx, m)
    out = out + a_coarse * (P @ CRx)
    return out


# ══════════════════════════════════════════════════════════════
# Graph Utilities — all O(E), never O(N²)
# ══════════════════════════════════════════════════════════════
...
def build_reverse_edge_map(edge_index):
    """O(E log E) via sorting — NO O(N²) lookup table."""
